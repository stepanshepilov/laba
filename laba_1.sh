...
case "$SOURCE_FILE" in
    *.c)
        ...
        ;;
    *.tex)
        # Запускаем pdflatex и сразу проверяем результат
        pdflatex "$SOURCE_NAME" >/dev/null 2>/tmp/err$$
        if [ $? -ne 0 ]; then
            # Если была ошибка - показываем ее и выходим
            cat /tmp/err$$ >&2
            rm -f /tmp/err$$
            clean_dir 10
        fi
        
        # Если мы здесь, значит pdflatex отработал успешно.
        # Теперь можно переименовывать файл.
